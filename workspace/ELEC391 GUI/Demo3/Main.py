...
def rectIntersect(pX, pY):
    ...
    if (pX >= sqX and pX <= cTRx) and (pY >= sqY and pY <= cBLy):
        ...
        if start_time2+time2_delay < time.time()*1000:
            ...
            stDesired1.set("%d", left_length)
            stDesired2.set("%d", right_length)            
            ser.write(b"\xFB")
...
def on_click(event):
    ...
    sqX = sqY = sqL = sqH = 0
    Set_PID(10,0,1)
    left_length, right_length = inverse_kinematic(mouse_x/x_conv - (x_range/2), y_end-mouse_y/y_conv)
    ser.write(b"\xFB")
    value = [int(left_length)]
    angle = bytes(value)
    ser.write(angle)
...
def inverse_kinematic(x,y):
    global base_length
    left_angle = math.atan(y/x)
    left_length = x/math.cos(left_angle)
    right_angle = math.atan(y/(base_length-x))
    right_length = y/math.sin(right_angle)
...
def MsgHandler(Msgtype, data):
    global left_length, right_length, startrx
